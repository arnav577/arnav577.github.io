...
function init(host) {
  const editorial = host.dataset.style === 'editorial';
  const neon = host.dataset.style === 'neon';
  ...
  const rubber = new THREE.MeshPhysicalMaterial({
    color: editorial ? 0x39485e : (neon ? 0x1a2210 : 0x11151d), metalness: 0.1, roughness: 0.55,
  ...
  function resize() {
    ...
    if (camera.aspect > 1.05) {
      if (editorial) { group.position.set(-0.6, 2.4, -3.2); group.scale.setScalar(0.72); }
      else if (neon) { group.position.set(3.5, 0.4, -2.6); group.scale.setScalar(0.8); }
      else { group.position.set(-3.6, 1.3, -1.2); group.scale.setScalar(1.0); }
    } else if (neon) {
      group.position.set(1.4, 3.5, -3.4);
